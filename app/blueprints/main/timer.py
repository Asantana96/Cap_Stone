...
class PomodoroTimer:
        ...
        if timer_id == 1:
            ...
            if not self.stopped:
                ...
                if self.chill_fish % 4 == 0:
                    self.tabs.select(1)
                    self.start_timer()
                    fish_info = self.get_fish_info()
                    if fish_info:
                        self.display_fish_info(fish_info)
                    else:
                        self.display_fish_info({'error': 'Fish not found'})
                else:
                    self.tabs.select(1)
                self.start_timer()
    ...
    def get_fish_info(self):
        try:
            response = requests.get('http://localhost:5000/fish')
            return response.json()
        except requests.exceptions.RequestException:
            return None

    def display_fish_info(self, fish_info):
        if 'error' in fish_info:
            self.reel_in_timer_label.config(text=f"Fish not found")
        else:
            fish_name = fish_info.get('name', 'Unknown')
            fish_color = fish_info.get('color', 'Unknown')
            fish_type = fish_info.get('type', 'Unknown')
            self.reel_in_timer_label.config(text=f"Fish: {fish_name}\nColor: {fish_color}\nType: {fish_type}")
